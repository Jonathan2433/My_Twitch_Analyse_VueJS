<template>
    <div>
        ...
        <form @submit.prevent="createPoll">
            ...
            <input
                type="checkbox"
                id="channel_points_voting_enabled"
                v-model="channel_points_voting_enabled"
                @change="updateChannelPointsPerVote"
            />

            <label htmlFor="channel_points_per_vote"> Nombre de point de chaîne par vote :</label>
            <!-- Si Checkbox channel_points_voting_enabled coché ALORS, le nombre de point est requis -->
            <input
                type="number"
                id="channel_points_per_vote"
                v-model="channel_points_per_vote"
                step="100"
                :required="channel_points_voting_enabled"
            />
...
export default {
    ...
    methods: {
        ...
        createPoll() {
            ...
            // Stockage des choix dans un tableau
            const choicesArray = this.choices.split(',').map((choice) => choice.trim())

            // Transformation du tableau en tableau associatif avec la bonne syntax pour l'API
            ...
            axios
                .post(`https://api.twitch.tv/helix/polls`, requestBody, {
                    headers: {
        ...
        // Méthode pour initialiser le nombre de points à 100 (minimum demandé par twitch) si sondage avec points
